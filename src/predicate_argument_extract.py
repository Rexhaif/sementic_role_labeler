...
from src.preposition_extract import in_complex_preposition, complex_preposition_child, get_children
...
class UdPredicateArgumentExtractor:
    ...
    def __init__(self, ud_model_path: str, lru_cache_size: int = 10000):
        self.pipeline = PipelineCommon([
            (
                ProcessorUDPipe("./data/models/russian-syntagrus-ud-2.5-191206.udpipe"),
                ['text'],
                {
                    "tokens" : "tokens",
                    "lemma" : "lemma",
    ...
    def extract(self, text: str):
        parse = self.call_pipeline(text)
        predicates = [Token(parse['tokens'][idx].text, idx) for idx in self.predicate_extractor(parse['postag'][0])]
        result = []
        for predicate in predicates:
            arguments = self.argument_extractor(
                predicate.position, parse['postag'][0],
                parse['morph'][0], parse['lemma'][0],
                parse['syntax_dep_tree'][0]
            )
            arguments = [Token(parse['tokens'][idx].text, idx) for idx in arguments]
            result.append({
                'predicate': predicate,
                'arguments': arguments
            })
